...
export function useGameListView() {
  ...
  const launchGame = async (game: any) => {
    try {
      ...
      showError(`启动游戏失败: ${error instanceof Error ? error.message : '未知错误'}`, {
        detail: error
      });
    }
  };
  ...
  const getGameIcon = (game: any) => {
    ...
    return iconMap[game.type] || '/images/null.svg';
  };
  ...
  return {
    ...
    isLoading,
    error,
    loadingState: { loading: isLoading.value, error: error.value }, // 兼容旧接口
