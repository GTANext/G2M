use crate::game::utils::{
    copy_dir_all, get_config_dir, read_g2m_json,
};
use crate::game::{
    ApiResponse, CopyImageResponse, GameInfo, ModInstallRequest, ModInstallResult,
};
use chrono::Utc;
...
pub async fn launch_game(
    ...
    use crate::game::process::ProcessService;
    let process_service = ProcessService::new();
    Ok(process_service.launch_game(&game_dir, &executable, run_as_admin))
}

/// 使用 ProcessService 打开游戏目录
#[tauri::command]
pub async fn open_game_folder(game_dir: String) -> Result<ApiResponse<()>, String> {
    use crate::game::process::ProcessService;
    let process_service = ProcessService::new();
    Ok(process_service.open_folder(&game_dir))
}
...
pub async fn save_game(
    ...
    use crate::game::services::GameService;
    let service = GameService::new(&app_handle)?;
    Ok(service.save(name, dir, exe, img, r#type))
}

/// 使用 GameService 获取所有游戏
#[tauri::command]
...
pub async fn update_game(
    ...
    use crate::game::services::GameService;
    let service = GameService::new(&app_handle)?;
    Ok(service.update(id, name, dir, exe, img, r#type, deleted))
}
...
pub async fn check_duplicate_directory(
    ...
    use crate::game::services::GameService;
    let service = GameService::new(&app_handle)?;
    Ok(service.check_duplicate_directory(&dir, exclude_game_id))
}
